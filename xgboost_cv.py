...
filenames = ('fe_test1.npz', 'fe_test2.npz', 'fe_test3.npz')
...
def cv_train(n):
    ...
    if n == 0: return load_pair(filenames[1], filenames[2])
    if n == 1: return load_pair(filenames[0], filenames[2])
    if n == 2: return load_pair(filenames[0], filenames[1])

def cv_test(n):
    """Returns the cross-validation test dataset for on of the splits."""
    assert n in (0, 1, 2)
    print('Loading test split', n)
    if n == 0: return load(filenames[0])
    if n == 1: return load(filenames[1])
    if n == 2: return load(filenames[2])

def cv_score(n):
    """Returns the cross-validation AUROC score on one of the splits."""
    x_train, y_train = cv_train(n)
    model = xgboost.XGBRegressor(max_depth=10, n_estimators=100, n_jobs=4)
    print('Training')
